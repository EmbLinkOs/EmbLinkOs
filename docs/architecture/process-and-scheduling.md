...
26. **Bug 25's "documented rule" was a rule, not a mechanism — and every later blocker violated it.** Bug 25 closed the IF-leak for `process_wait()` and stated "any future voluntary-block path needs the same." Ten `sched_block_current_locked()` call sites then accumulated (fs wait-queue, pipes ×2, keyboard ×2, channels ×2, endpoints, ksync, kworker) and **none** restored IF — each written against the primitive's comment, which described the lock contract but not the flags contract. Symptom: a heavy kernel-context write workload (`test writestorm`, 200 create/write/close/unlink cycles) froze the whole machine within ~25 iterations — the storm's fs writes contend the FS wait-queue/bounce paths against the kworker's deferred closes; whichever sleeper was resumed by a *timer-initiated* switch-in woke with IF=0 (verbatim-flags kcontext + `g_sched_lock.saved_flags` last written by the ISR's own `spin_lock`), carried it into `ata_wait_irq`, and executed `hlt` with interrupts off — permanent sleep, single core, machine dead. Probabilistic (needs the timer to be the switch-in initiator), which made it masquerade as "tcc hangs on-OS" for a whole debugging arc. Pinned by dumping RIP/RFLAGS through the QEMU monitor at the hang (`RIP` = ata.c's `hlt`, `RFL=0x2`) and an IF-probe at `ata_wait_irq` entry. **Fix, structural this time:** the `sti` lives **inside `sched_block_current_locked()` itself** — every current and future caller inherits it, and the rule can no longer be forgotten (unconditional by contract: callers are voluntary process-context sleepers; an IRQ handler cannot sleep, and no sleeper can mean to hold IF=0 across a block it expects to be woken from). Defense-in-depth: `ata_wait_irq` now uses the canonical `sti;hlt` pair (its loop's own precondition) plus a loud entry canary, and `test writestorm` asserts zero IF leaks across 200 cycles. **The general shape to recognize:** a rule documented at one fix site does not protect the *next* author, who reads the primitive they call, not the ledger — when a rule is about every caller of X, it belongs inside X.
