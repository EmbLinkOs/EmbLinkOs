...
| **Screen lock** | ✅ **done** — EmbLink menu → Lock Screen. The desktop draws it on the layer `embk_win_desktop_front()` lifts above every window, because a lock has to be in front of everything and an ordinary window can be lowered; the password is checked by **authd**, which init starts OUTSIDE the session and tells whose session it is, because /etc/shadow is outside every session's namespace on purpose and authd will answer about that one account only. Wrong answers cost the greeter's own escalating delay. The kernel stops honouring GUI+Tab / GUI+W / GUI+Q while the layer is up — it performs them itself, so nothing on screen could decline them, and GUI+Tab over a lock would raise an app window, which IS the session. `make test-lock` types an account into first-boot setup on the real keyboard, signs in, locks from the menu, measures that the dock and the desktop icons are gone, presses GUI+Tab and reads the kernel's refusal, is refused a wrong password BY AUTHD, and comes back with the right one. ⚠️ **No idle auto-lock yet** — it locks when you ask it to, not when you walk away | Walking away from the machine leaves the session open |
| **Real HiDPI scaling** | `ui_scale` exists, clamped to 80–130% | A modern laptop panel needs 200%; the desktop looks half-size |
| **Time: NTP + time zones** | ✅ **done** — SNTP (`kernel/net/ntp/`) runs on a kernel thread as soon as the network comes up, off the boot path, and steps the clock by an OFFSET rather than writing the CMOS chip back (`rtc_set_unix`; the PL031 driver learned the same offset, where it matters more since a board with no battery reads 1970). Zones are a POSIX TZ string per entry in `user/lib/oscfg.h` — the rule itself, not an Olson database — picked by region and city in Settings (Date & Time), applied by every process that reads the preferences, and handed to every application the desktop launches, since nothing in this system inherits an environment it was not given. `make test-ntp` breaks the guest's clock by four hundred days and measures what SNTP leaves behind, with the host's clock bracketing the run so a wrong epoch constant cannot pass by being wrong twice; `make test-tz` checks the conversions at two fixed instants in both hemispheres' summers; `make test-timezone` picks Tokyo in Settings and watches the desktop adopt JST-9. ⚠️ No drift discipline (one step at boot, not a slew), and no zone history — a TZ string knows this year's rule, not 1974's | The clock drifts and is in the wrong zone |
