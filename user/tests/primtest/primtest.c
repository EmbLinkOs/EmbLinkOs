/* user/tests/primtest/primtest.c — the EmbLink native-primitive self-test (formerly
 * misnamed init.c/init.elf; the real pid-1 init now owns that name — see
 * user/system/init/init.c and docs/USERSPACE_v2.md UP1). Freestanding: its own _start
 * (no crt0), no libc. Built entirely on the EmbLink SDK (user/embk.h) rather
 * than hand-rolled syscalls, so it doubles as the SDK's own dogfooding. Linked
 * at 0x400000 (user/user.ld). Spawned by `test ring3 threads` / `test ring3`.
 *
...
static int spawn_test(void)
{
    ...
    long handle = embk_spawn("/system/bin/primtest.elf", argv, actions, 1);
...
static int env_spawn_test(void)
{
    ...
    long handle = embk_spawn_env("/system/bin/primtest.elf", argv, envp, actions, 1);
...
static int cancel_test(void)
{
    ...
    long h = embk_spawn("/system/bin/primtest.elf", argv, acts, 1);
...
static void ctrlc_parent(void)
{
    ...
    long h = embk_spawn("/system/bin/primtest.elf", argv, (void *)0, 0);
...
static void surface_parent(void)
{
    ...
    char *cargv[] = { "/system/bin/primtest.elf", "surface-child", idxbuf, (char *)0 };
    ...
    long h = embk_spawn("/system/bin/primtest.elf", cargv, act, 1);
...
static void compositor_role(void)
{
    ...
    long spawn_h = embk_spawn("/system/bin/primtest.elf", cargv, 0, 0);
